...
const NavItems = ({ ToggleMenu, MenuOpen, isActive, }) => {
  ...
  return (
    <>
      <nav className={`${MenuOpen ? 'translate-x-0' : '-translate-x-full'} duration-300 mx-auto text-white  poppins-medium bg-gradient-to-b from-[#A405A7] to-[#400241]  min-h-65 max-h-screen max-w-full w-full top-0 right-0 absolute z-50`}>
        ...
        <div className="p-5">
          ...
          <ul className="flex flex-col items-start mt-5 *:my-2 *:px-2  overflow-y-auto max-h-[calc(100vh-150px)] ">
            <Link to="/" className={` ${isActive == '/' ? 'text-[#A405A7] bg-white  rounded-lg min-w-[75px] py-1  px-2' : ' '}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
                }}
            ...
            <li ref={dropdown1Ref} >
              <Link onClick={() => setDropdown1(!dropdown1)} className={`flex items-center gap-1  ${about.some(link => location.pathname === link.path)
                ? 'text-[#A405A7] bg-white  py-1 px-1 rounded-lg'
                : ''
                }`}>
                About Us
                <FaChevronDown
            ...
            <Link to="/contact-us" className={` ${isActive == '/contact-us' ? 'text-[#A405A7] bg-white  rounded-lg min-w-[75px] py-1  px-2' : ''}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
